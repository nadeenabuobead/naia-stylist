...
subCode guidance:
- no_garment_visible: use ONLY when there is genuinely no wearable fashion item anywhere in the image (e.g. a landscape, a plain background with no item, or a completely unrelated object like furniture or food). Do NOT use this code merely because the item is worn by a person or displayed on a mannequin — those are valid photos. Crucially, do NOT use this code for accessories, jewellery, watches, bags, shoes, belts, hats, sunglasses, or scarves — these are all valid fashion items even when no clothing is visible in the same photo.
