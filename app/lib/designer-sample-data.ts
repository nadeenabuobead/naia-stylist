    ...
    designActions: [
      {
        ...
        product: SEEN,
        collection: "workwear",
        observedEvidence: `100% feeling achieved among Corporate Chic (${pm[SEEN].wrCount}/${pm[SEEN].wrCount} post-wear)`,
      ...
      },
      {
        ...
        product: WHOLE,
        collection: "everyday",
        observedEvidence: `${pm[WHOLE].saveCount} save-intent outcomes · ${pm[WHOLE].buyCount} buy-intent outcomes · save/buy-intent gap is the widest in the collection · Top feedback: 'Not sure when to wear it'`,
      ...
      },
      {
        ...
        product: GROUNDED,
        collection: "workwear",
        observedEvidence: `${pm[GROUNDED].objectionCount} fit objections · Top: "${pm[GROUNDED].topObjection}" · ${pm[GROUNDED].sessionCount} sessions · Buy-intent conversions visible when fit resolves`,
  ...
  const opportunityFeed = [
    {
      ...
      evidence: `${pm[SEEN].sessionCount} sessions · ${pm[SEEN].buyCount} Buy/Skip buy-intent outcome${pm[SEEN].buyCount !== 1 ? "s" : ""} · ★${(pm[SEEN].avgRating as number).toFixed(1)} avg (${pm[SEEN].sampleSize} review${pm[SEEN].sampleSize !== 1 ? "s" : ""}) · ${Math.round(pm[SEEN].rewearRate * 100)}% rewear (${pm[SEEN].wrCount}/${pm[SEEN].wrCount} post-wear)`,
    ...
    },
    {
      ...
      confidence: evidenceConfidence(pm[WHOLE].evidenceN),
      estimatedCommercialRelevance: "high",
      insight: `Becoming Whole has ${pm[WHOLE].saveCount} save-intent and ${pm[WHOLE].buyCount} buy-intent outcomes — the widest save/buy-intent gap in the collection`,
      customerNeed: "Customers are drawn to the piece but need confidence in when and how to wear it before purchasing",
      evidence: `${pm[WHOLE].saveCount} save-intent outcomes · ${pm[WHOLE].buyCount} buy-intent outcomes · top objection: 'Not sure how to style it'`,
      timePeriod: periodLabel,
    ...
    },
    {
      ...
      evidence: `${groundedObjCount} fit objections (SS events) · top barrier: "${pm[GROUNDED].topObjection}"${pm[GROUNDED].buyCount > 0 ? ` · ${pm[GROUNDED].buyCount} buy-intent outcome${pm[GROUNDED].buyCount > 1 ? "s" : ""} when fit resolves` : " · buy-intent conversions visible in 90D+ window"}`,
      timePeriod: periodLabel,
  ...
  const advanced = {
    ...
    collectionHealth: {
      ...
      factors: {
        ...
        emotionalOutcomes:      { score: nwr > 0 ? pct(ejAchieved, nwr) : 0, label: `${nwr > 0 ? pct(ejAchieved, nwr) : 0}% feeling achievement rate (${ejAchieved}/${nwr} post-wear reviews)`, confidenceLabel: nwr === 0 ? "Not measured" : nwr === 1 ? "Single observation" : nwr < 5 ? "Directional signal" : nwr < 10 ? "Emerging pattern" : nwr < 20 ? "Established pattern" : "Strong pattern", weight: 20 },
