...
export default function AiutaUploadTestPage() {
  ...
  async function handleCreateSku() {
    ...
    const response = await fetch("/api/aiuta-upload-test", {
      method: "POST",
      body: formData,
    });

    const text = await response.text();
    setCreateSkuResult(text);
  }

  async function handleUpload() {
    if (!file) {
      alert("Please choose a person photo first.");
      return;
    }
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      if (parsed.operation_id) {
        setOperationId(parsed.operation_id);
      } else if (parsed.id) {
  ...
  return (
    <div style={{ padding: 24, fontFamily: "Arial, sans-serif" }}>
      <h1>Aiuta Test</h1>
      <p>0. Create SKU. 1. Upload person photo. 2. Generate with SKU. 3. Check status.</p>
