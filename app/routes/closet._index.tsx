...
export async function loader({ request }: LoaderFunctionArgs) {
  try {
    ...
    return data({ items: [], authenticated: false, error: err?.message });
  }
}
    ...
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}>
        <div style={{ textAlign: "center", maxWidth: "24rem" }}>
          ...
          <a href="https://naiabynadine.com/account/login" style={{ padding: "0.75rem 1.5rem", background: "#c4a0a0", color: "white", borderRadius: "9999px", textDecoration: "none", fontWeight: 500 }}>
            Log In
          </a>
        </div>
      </div>
    ...
    <div style={{ minHeight: "100vh", background: "#faf9f7" }}>
      <header style={{ padding: "1rem", background: "white", position: "sticky", top: 0, zIndex: 10 }}>
        <div style={{ maxWidth: "32rem", margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <button onClick={() => setShowAddForm(!showAddForm)} style={{ fontSize: "0.875rem", fontWeight: 500, color: "#c4a0a0", background: "none", border: "none", cursor: "pointer" }}>
            + Add
          </button>
        </div>
      </header>
      ...
      <main style={{ padding: "1rem", maxWidth: "32rem", margin: "0 auto" }}>
          ...
          <div style={{ background: "white", borderRadius: "0.75rem", padding: "1rem", marginBottom: "1rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
              <input
                ...
                style={{ padding: "0.75rem", borderRadius: "0.5rem", border: "1px solid #e5e5e5", fontSize: "0.875rem", width: "100%", boxSizing: "border-box" as const }}
              />
              <select
                value={newCategory}
                onChange={(e) => setNewCategory(e.target.value)}
                style={{ padding: "0.75rem", borderRadius: "0.5rem", border: "1px solid #e5e5e5", fontSize: "0.875rem", background: "white" }}
              >
              ...
              <div style={{ display: "flex", gap: "0.5rem" }}>
                <button onClick={handleAdd} disabled={!newName} style={{ flex: 1, padding: "0.75rem", background: "#c4a0a0", color: "white", border: "none", borderRadius: "0.5rem", fontWeight: 500, cursor: "pointer" }}>
                  Add to Closet
                </button>
                <button onClick={() => setShowAddForm(false)} style={{ padding: "0.75rem 1rem", background: "transparent", color: "#888", border: "1px solid #e5e5e5", borderRadius: "0.5rem", cursor: "pointer" }}>
                  Cancel
                </button>
              </div>
            </div>
        ...
        <div style={{ display: "flex", gap: "0.5rem", overflowX: "auto", paddingBottom: "0.5rem", marginBottom: "1rem" }}>
          {["ALL", ...CATEGORIES].map((cat) => (
            <button
              ...
              style={{ padding: "0.375rem 0.875rem", borderRadius: "9999px", fontSize: "0.75rem", fontWeight: 500, whiteSpace: "nowrap" as const, border: "none", cursor: "pointer", background: activeCategory === cat ? "#2d2d2d" : "white", color: activeCategory === cat ? "white" : "#888" }}
            >
        ...
        {filtered.length === 0 && (
          <div style={{ textAlign: "center", padding: "3rem 1rem" }}>
            ...
            <button onClick={() => setShowAddForm(true)} style={{ padding: "0.75rem 1.5rem", background: "#c4a0a0", color: "white", border: "none", borderRadius: "9999px", fontWeight: 500, cursor: "pointer" }}>
