...
function TrendReportDisplay({ report, query }) {
  ...
  let currentContent = [];
  let trendLensContent = '';
  
  lines.forEach(line => {
    if (line.startsWith('## ')) {
      if (currentSection) {
        if (currentSection === 'YOUR TREND LENS') {
          trendLensContent = currentContent.join('\n');
        } else {
    ...
    <div style={{ 
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          Personalized for you • {new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
        </div>
      </div>
                      ...
                      <p key={i} style={{ margin: "0 0 12px" }}>
                        {parts.map((part, idx) => 
                          part.startsWith('**') && part.endsWith('**') 
                            ? <strong key={idx} style={{ fontWeight: 600 }}>{part.replace(/\*\*/g, '')}</strong>
                            : part
                        )}
                      </p>
                    );
      ...
      <div style={{ 
        ...
        nAia Fashion Intelligence • Powered by your style DNA
      </div>
    </div>
  );
}
  ...
  const callAI = async () => {
    ...
    try {
      const res = await fetch("/api/style", { method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ mode, outfit, mood, feeling, event, styleDNA, vibe, styleDNA, bodyPref, styleIntelligence: customer?.styleIntelligence, previousPieces, closetItem: itemsToStyle[0] || null, closetItems: itemsToStyle, naiaPiece: naiaPiece ? { name: naiaPiece.name || naiaPiece.title, category: naiaPiece.category || naiaPiece.type || "", stylingNotes: naiaPiece.stylingNotes || "", moodMatch: naiaPiece.moodMatch || "", stylingRole: naiaPiece.stylingRole || "", statementLevel: naiaPiece.statementLevel || "", occasion: naiaPiece.occasion || "", sihouette: naiaPiece.sihouette || "" } : null, closet: closet.map(i => ({ name: i.name, category: i.category })) }),
      });
      const data = await res.json();
            ...
            {customer && (
              <>
                ...
                <a href="/closet" style={s.iconBtn} title="My Closet">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="4" y="3" width="16" height="18" rx="1"/><line x1="12" y1="3" x2="12" y2="21"/><circle cx="10" cy="12" r="0.5" fill="currentColor"/><circle cx="14" cy="12" r="0.5" fill="currentColor"/></svg>
                </a>
                      ...
                      {naiaProducts.filter(p => parsedResult.naiaRecommendations.some(r => r.toLowerCase().includes(p.title.toLowerCase()))).map(p => {
                        ...
                        return (
                          <div key={p.id} style={{ border: "1px solid #d4cfc9", borderRadius: "2px", overflow: "hidden", position: "relative" }}>
                            <a href={p.url} target="_blank" rel="noreferrer" style={{ textDecoration: "none", color: "inherit" }} onClick={async () => {
                              if (customerToken && lastHistoryId) {
                                try {
                                  await fetch("/api/wishlist", {
                                    method: "POST",
                                    headers: { "Content-Type": "application/json", "Authorization": `Bearer ${customerToken}` },
                                    body: JSON.stringify({ action: "track", sessionId: lastHistoryId, naiaProductId: String(p.id), title: p.title, eventType: "clicked" })
                                  });
                                } catch {}
                              }
                            }}><img src={p.image} alt={p.title} style={{ width: "100%", height: "140px", objectFit: "cover", display: "block" }} /><div style={{ padding: "8px", fontSize: "12px" }}>{p.title}</div></a>
