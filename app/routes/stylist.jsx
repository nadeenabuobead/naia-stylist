...
function parseStylingResult(text) {
  ...
  for (const line of lines) {
    ...
    if (lower.startsWith("you're feeling:") || lower.startsWith("you're feeling:")) { sections.feelingNow = line.split(":").slice(1).join(":").trim(); currentSection = ""; continue; }
    if (lower.startsWith("you want to feel:")) { sections.feelingNext = line.split(":").slice(1).join(":").trim(); currentSection = ""; continue; }
    ...
    if (lower === "shift" || lower.startsWith("shift:")) { currentSection = "shift"; const a = line.split(":").slice(1).join(":").trim(); if (a) sections.shift = a; continue; }
    if (line.startsWith("-")) { const c = line.replace(/^-+\s*/, "").trim(); if (currentSection === "outfitDirection") sections.outfitDirection.push(c); if (currentSection === "whyThisWorks") sections.whyThisWorks.push(c); continue; }
...
export default function Stylist() {
  ...
  const s = {
    ...
    dot: (active, done) => ({ width: active ? "24px" : "8px", height: "8px", borderRadius: "4px", background: done ? "#1a1816" : active ? "#1a1816" : "#d4cfc9", transition: "all 0.3s" }),
    title: { fontSize: "13px", letterSpacing: "0.2em", textTransform: "uppercase", color: "#8a7f75", marginBottom: "12px", fontFamily: '"Cormorant Garamond", Georgia, serif' },
    bigQ: { fontSize: "32px", lineHeight: 1.2, fontWeight: 400, margin: "0 0 32px", fontStyle: "italic" },
    input: { width: "100%", padding: "16px 0", borderBottom: "1px solid #1a1816", background: "transparent", border: "none", borderBottom: "1px solid #c8c2bb", fontSize: "18px", color: "#1a1816", outline: "none", fontFamily: '"Cormorant Garamond", Georgia, serif', boxSizing: "border-box" },
    ...
    productCard: (active) => ({ border: `2px solid ${active ? "#1a1816" : "transparent"}`, borderRadius: "2px", cursor: "pointer", overflow: "hidden", position: "relative" }),
      ...
      <div style={s.container}>
        ...
        {step === 1 && (
          <div>
            ...
            <input
              style={s.input}
              placeholder="overwhelmed, tired, excited..."
              value={mood}
        ...
        {step === 2 && (
          <div>
            ...
            <input
              style={s.input}
              placeholder="powerful, calm, seen, soft..."
              value={feeling}
        ...
        {step === 6 && (
          <div>
            ...
            <div onClick={() => setMode("closet_only")} style={s.modeCard(mode === "closet_only")}>
              <div style={{ fontSize: "17px", fontWeight: 500, marginBottom: "4px" }}>Style pieces from my closet together</div>
              <div style={{ fontSize: "14px", opacity: 0.7 }}>Pick 2–3 pieces you own and we'll build an outfit from them</div>
            </div>
        ...
        {step === 7 && (
          <div>
            ...
            <p style={s.bigQ}>
              {mode === "closet_naia" ? "Pick your closet piece + a nAia piece" :
               mode === "recommend_naia" ? "Pick a piece from your closet" :
               "Pick 2–3 pieces from your closet"}
            </p>
            ...
            <div style={{ marginBottom: "32px" }}>
              ...
              <div style={s.closetGrid}>
                {closet.map(piece => (
                  <div key={piece.id} onClick={() => toggleClosetItem(piece.id)}
                    style={{ border: `2px solid ${selectedClosetIds.includes(piece.id) ? "#1a1816" : "#d4cfc9"}`, borderRadius: "2px", overflow: "hidden", cursor: "pointer" }}>
                    {piece.image
              ...
              {/* Add item */}
              {!showAddItem ? (
                <button style={{ ...s.outlineBtn, marginTop: "16px", fontSize: "12px" }} onClick={() => setShowAddItem(true)}>+ Add piece</button>
              ) : (
              ...
              <div style={{ marginBottom: "32px" }}>
                ...
                {currentNaiaPiece && (
                  <div onClick={() => setSelectedNaiaPiece(selectedNaiaPiece?.id === currentNaiaPiece.id ? null : currentNaiaPiece)}
                    style={{ ...s.productCard(selectedNaiaPiece?.id === currentNaiaPiece.id), display: "flex", gap: "12px", padding: "12px", border: `2px solid ${selectedNaiaPiece?.id === currentNaiaPiece.id ? "#1a1816" : "#d4cfc9"}`, marginBottom: "16px" }}>
                ...
                <div style={s.productGrid}>
                  {naiaProducts.map(p => (
                    <div key={p.id} onClick={() => setSelectedNaiaPiece(selectedNaiaPiece?.id === p.id ? null : p)}
                      style={s.productCard(selectedNaiaPiece?.id === p.id)}>
          ...
          <div>
            ...
            {loading ? (
              ...
              <div>
                ...
                <div style={{ marginTop: "32px" }}>
                  <button style={s.outlineBtn} onClick={() => { setStep(1); setStylingResult(""); setSelectedClosetIds([]); setSelectedNaiaPiece(null); setMood(""); setFeeling(""); setEvent(""); setStyleWords([]); setBodyPref(""); setMode(""); }}>Start over</button>
                </div>
              </div>
            ) : (
              <div>
                <p style={{ fontSize: "18px", fontStyle: "italic", whiteSpace: "pre-line" }}>{stylingResult}</p>
